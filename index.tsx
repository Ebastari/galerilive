...
const GalleryApp: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white selection:bg-blue-500/30">
      {/* Navigation Header */}
      <nav className="sticky top-0 z-[100] bg-slate-950/80 backdrop-blur-xl border-b border-white/5">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between gap-6">
              ...
              <div className="flex-1 max-w-md relative">
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl py-3.5 px-12 text-xs font-bold text-white outline-none focus:ring-4 focus:ring-blue-500/20 transition-all"
                  value={search}
              ...
              <button 
                onClick={loadData}
                className={`p-3.5 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all ${isLoading ? 'animate-spin' : ''}`}
              >
            ...
            <div className="flex flex-wrap items-center gap-3">
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-xl py-2 px-4 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none cursor-pointer hover:bg-white/10 transition-all"
              >
                <option value="" className="bg-slate-900">Semua Pengawas</option>
                {filterOptions.pengawas.map(p => <option key={p} value={p} className="bg-slate-900">{p}</option>)}
              </select>
              ...
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-xl py-2 px-4 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none cursor-pointer hover:bg-white/10 transition-all"
              >
                <option value="" className="bg-slate-900">Semua Jenis Bibit</option>
                {filterOptions.bibit.map(b => <option key={b} value={b} className="bg-slate-900">{b}</option>)}
              </select>
              ...
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-xl py-2 px-4 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-blue-500/50 appearance-none cursor-pointer hover:bg-white/10 transition-all"
              >
                <option value="" className="bg-slate-900">Semua Tanggal</option>
                {filterOptions.tanggal.map(t => <option key={t} value={t} className="bg-slate-900">{t}</option>)}
              </select>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {filtered.map((item, idx) => (
              <div 
                ...
                className="group relative aspect-[3/4] bg-slate-900 rounded-[2.5rem] overflow-hidden border border-white/5 cursor-pointer shadow-2xl hover:-translate-y-2 transition-all duration-500"
          ...
          <div className="p-8 md:p-12 bg-slate-900 border-t border-white/5 bg-gradient-to-t from-black to-slate-900/50">
            <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-center gap-10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full lg:w-auto text-center">
                ...
                <div className="bg-white/5 p-5 rounded-[2rem] border border-white/5"><p className="text-[9px] font-black text-slate-500 uppercase mb-2">Tanggal</p><p className="text-[10px] font-black leading-tight mt-1">{activeItem.Tanggal}</p></div>
